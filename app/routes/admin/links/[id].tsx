...
export default createRoute(async (c) => {
  ...
  if (c.req.method === 'POST') {
    ...
    if (body._action === 'UPDATE') {
      ...
      await c.env.DB.prepare('UPDATE links SET slug=?, target_url=?, og_title=?, og_description=?, og_image_url=? WHERE id=?')
        .bind(slug, body.target_url as string, body.og_title as string, body.og_description as string, body.og_image_url as string, id).run()
  ...
  return c.render(
    <AdminShell activePage="links">
      <div className="mb-8 border-b pb-4 flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-800">Edit: /{link.slug}</h1>
